...
Функция СоздатьДокументПоРаспознаннымДанным(ХешСумма, Идентификатор, ДополнительныеПараметры, ОбновитьДокумент = Ложь) Экспорт
	...
	ДанныеРаспознавания = ДанныеОдногоФайлаПоХешСумме(ХешСумма, Идентификатор);
	
	СериализоватьВсеДанныеРаспознавания(ДанныеРаспознавания);
...
Функция СоздатьКонтрагентаПоРаспознаннымДанным(Знач ЗаписьРегистра) Экспорт
	...
	ДанныеРаспознавания = ДанныеОдногоФайлаПоХешСумме(ЗаписьРегистра.ХешСумма, ЗаписьРегистра.Объект);
...
Функция ПолучитьДанныеРегистраПоХешСумме(ХешСумма, Идентификатор = Неопределено) Экспорт
	...
	|	ИЛИ &НеОтбиратьПоОбъекту)";
	
	Запрос.УстановитьПараметр("ХешСумма", ХешСумма);
	Запрос.УстановитьПараметр("Объект", Идентификатор);
	Запрос.УстановитьПараметр("НеОтбиратьПоОбъекту", Идентификатор = Неопределено);
...
Процедура СериализоватьВсеДанныеРаспознавания(ДанныеРаспознавания)
...
Процедура НайтиСвязиПоКлючевымРеквизитам(ЗаписьРегистра, ЗагрузкаНовогоДокумента = Ложь)
	...
	СоздаватьКонтрагентаАвтоматически = ПолучитьНастройкуРаспознавания("СоздаватьКонтрагентовАвтоматическиПриЗагрузке") = Истина;
	...
	ОпределитьОрганизацию(ЗаписьРегистра, РеквизитПоискаКонфигурации, СокрЛП(СтруктураКлючевыхРеквизитов[ИмяПоля_ИД_Поставщика]), ТочностьСтрогогоПоиска, ЭтоИсходящийДокумент, Истина);
	ОпределитьОрганизацию(ЗаписьРегистра, РеквизитПоискаКонфигурации, СокрЛП(СтруктураКлючевыхРеквизитов[ИмяПоля_ИД_Клиента]), ТочностьСтрогогоПоиска, ЭтоИсходящийДокумент, Ложь);
	...
	Если ЭтоИсходящийДокумент Тогда
		...
		ИННОрганизации = СтруктураКлючевыхРеквизитов[ИмяПоля_ИД_Поставщика];
		ИННКонтрагента = СтруктураКлючевыхРеквизитов[ИмяПоля_ИД_Клиента];
		
	Иначе
		
		ИННОрганизации = СтруктураКлючевыхРеквизитов[ИмяПоля_ИД_Клиента];
		ИННКонтрагента = СтруктураКлючевыхРеквизитов[ИмяПоля_ИД_Поставщика];
		
	КонецЕсли;
	
	Если ЗагрузкаНовогоДокумента Тогда
	
		Если НЕ ЗначениеЗаполнено(ЗаписьРегистра.Контрагент) И СоздаватьКонтрагентаАвтоматически Тогда
			
			ЗаписьРегистра.Контрагент = СоздатьКонтрагентаПоРаспознаннымДанным(ЗаписьРегистра);			
	...
	Если НЕ ЗагрузкаНовогоДокумента Тогда
		...
		Если ТочностьСтрогогоПоиска < ПорогПоискаИИ Тогда
			...
			СтруктураКлючевыхРеквизитов[ИмяПоля_ИД_Поставщика] = ИННОрганизации;
			СтруктураКлючевыхРеквизитов[ИмяПоля_ИД_Клиента] = ИННКонтрагента;
...
Процедура ОпределитьОрганизацию(ЗаписьРегистра, РеквизитПоискаКонфигурации, ИНН_ОКПО, ТочностьСтрогогоПоиска, ФлагИсходящийДокумент, ПереданИННПоставщика)
	...
	|ГДЕ
	|	Организации.ИНН = &ИНН_ОКПО";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Организации.ИНН", "Организации." + РеквизитПоискаКонфигурации);
	
	Запрос.Параметры.Вставить("ИНН_ОКПО", ИНН_ОКПО);
...
Процедура ОпределитьКонтрагента(ЗаписьРегистра, РеквизитПоискаКонфигурации, ИНН_ОКПО, ТочностьСтрогогоПоиска, ФлагИсходящийДокумент, ПереданИННКлиента)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Контрагенты.ИНН", "Контрагенты." + РеквизитПоискаКонфигурации);
...
Функция ДанныеОдногоФайлаПоХешСумме(ХешСумма, Идентификатор = Неопределено)
