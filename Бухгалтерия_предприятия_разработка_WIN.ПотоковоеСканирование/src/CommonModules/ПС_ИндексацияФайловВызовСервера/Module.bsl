...
Функция СоздатьДокументПоРаспознаннымДанным(ХешСумма, ИД, ДополнительныеПараметры, ОбновитьДокумент = Ложь) Экспорт
	...
	ДанныеРаспознавания = ДанныеОдногоФайлаПоХешСумме(ХешСумма, ИД);
...
Процедура ПривязатьЗаписьРегистраКСозданномуДокументу(ХешСумма, ИД, СсылкаНаДокумент) Экспорт
	...
	Для каждого ТекЗапись Из НаборЗаписейРегистра Цикл
		...
		Если ТекЗапись.Объект = ИД Тогда
...
Процедура ОбработатьДокумент(ХешСумма, АдресФайлаНаКлиенте = "", Знач РучнойЗапуск = Ложь, ИД = Неопределено) Экспорт
	...
	Если ИД <> Неопределено Тогда
		...
		НаборЗаписейРегистра.Отбор.Объект.Установить(ИД);
...
Функция ПолучитьДанныеРегистраПоХешСумме(ХешСумма, ИД = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("Объект", ИД);
	Запрос.УстановитьПараметр("НеОтбиратьПоОбъекту", ИД = Неопределено);
...
Процедура НайтиСвязиПоКлючевымРеквизитам(ЗаписьРегистра, ЗагрузкаНовогоДокумента = Ложь)
	...
	Если ЗначениеЗаполнено(СтруктураКлючевыхРеквизитов.OKUD) Тогда
		...
		//!!! Могут быть определены индивидуальные правила поиска и создания, например для формы КС
...
Функция ДанныеОдногоФайлаПоХешСумме(ХешСумма, ИД = Неопределено)

	//Получаем файл
	ДанныеФайлаМассив = ПолучитьДанныеРегистраПоХешСумме(ХешСумма, ИД);
