...
Процедура ВидимостьКнопкиПодтвердить()
	...
	Элементы.ПодтвердитьИПрикрепить.Видимость = ЗначениеЗаполнено(Запись.Объект) 
	И ТипЗнч(Запись.Объект) <> Тип("Строка")
	И (НЕ Запись.Прикреплен ИЛИ НЕ Запись.ОбработкаЗавершена);
