...
Процедура ОбработатьДокумент(ХешСумма, АдресФайлаНаКлиенте = "", ПрикрепитьДокументНаКлиенте = Ложь) Экспорт
	...
	Если НаборЗаписейРегистра.Количество() > 0 Тогда
		...
		Если ЗаписьРегистра.Найден Тогда
			...
			Если ЗаписьРегистра.Вероятность >= ПорогПрикрепленияФайла Тогда
				...
				Иначе
					...
					ПрикрепленУспешно = ПрикрепитьСканДокументаНаСервере(ЗаписьРегистра, ЗаписьРегистра.ИмяФайла); //Прикрепляем файл, который хранится на сервере
					
					Если ПрикрепленУспешно Тогда
...
Процедура ПрикрепитьИЗавершитьОбработкуДокумента(ХешСумма, АдресХранилища) Экспорт
	...
	Если НаборЗаписейРегистра.Количество() > 0 Тогда
		...
		ПрикрепленУспешно = ПрикрепитьСканДокументаНаСервере(ЗаписьРегистра, АдресХранилища); //Прикрепляем файл, который хранится на сервере
...
Функция ПрикрепитьСканДокументаНаСервере(ЗаписьРегистра, АдресХранилищаИлиПуть) Экспорт
