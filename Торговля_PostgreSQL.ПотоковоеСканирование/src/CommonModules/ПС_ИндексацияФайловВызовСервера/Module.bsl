...
Процедура ОбработатьДокумент(ХешСумма, АдресФайлаНаКлиенте = "") Экспорт
	...
	РезультатМассив = ПолучитьДанныеРегистраПоХешСумме(ХешСумма);
	
	Если РезультатМассив.Количество() > 0 Тогда
	
		Если НЕ РезультатМассив[0].Распознан Тогда
			
			РезультатРаспознавания = РаспознатьДокумент(РезультатМассив[0], АдресФайлаНаКлиенте);
...
Функция РаспознатьДокумент(Выборка, АдресФайлаНаКлиенте = "")
	...
	Если Выборка.НаКлиенте И ЭтоАдресВременногоХранилища(АдресФайлаНаКлиенте) Тогда
		...
		Если ТипЗнч(ДанныеИзХранилища) = Тип("Массив") Тогда //Выб сконвертирован на клиенте
		
			ТабличныйДокумент = ПереупаковатьPDF(ДанныеИзХранилища);
			
			ИмяВременногоФайла = ПолучитьИмяВременногоФайла("pdf");	
			ТабличныйДокумент.Записать(ИмяВременногоФайла, ТипФайлаТабличногоДокумента.PDF);
			
		Иначе //Сохраняем и конвертируем на сервере
		
			ИмяВременногоФайла = ПолучитьИмяВременногоФайла();
			ДвоичныеДанные = ДанныеИзХранилища;
			ДвоичныеДанные.Записать(АдресФайлаНаКлиенте);
			
			//!!! Тут нужно переконвертировать файл на сервере
			//но пока временно просто передаем на распознование файл без конвертации
