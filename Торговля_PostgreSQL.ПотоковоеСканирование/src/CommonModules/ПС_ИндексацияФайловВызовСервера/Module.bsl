...
Процедура ОбработатьДокумент(ХешСумма, АдресФайлаНаКлиенте = "", РучнойЗапуск = Ложь) Экспорт
	...
	Если НаборЗаписейРегистра.Количество() > 0 Тогда
		...
		Если НЕ РучнойЗапуск И ЗаписьРегистра.КоличествоПопыток > ПорогПопыток Тогда
...
Процедура НайтиСвязиПоКлючевымРеквизитам(ЗаписьРегистра)
	...
	ОпределитьОрганизацию(ЗаписьРегистра, СокрЛП(СтруктураКлючевыхРеквизитов.INN_company), ТочностьСтрогогоПоиска, ЭтоИсходящийДокумент, Истина);
	ОпределитьОрганизацию(ЗаписьРегистра, СокрЛП(СтруктураКлючевыхРеквизитов.INN_client), ТочностьСтрогогоПоиска, ЭтоИсходящийДокумент, Ложь);

	ОпределитьКонтрагента(ЗаписьРегистра, СокрЛП(СтруктураКлючевыхРеквизитов.INN_client), ТочностьСтрогогоПоиска, ЭтоИсходящийДокумент, Истина);
	ОпределитьКонтрагента(ЗаписьРегистра, СокрЛП(СтруктураКлючевыхРеквизитов.INN_company), ТочностьСтрогогоПоиска, ЭтоИсходящийДокумент, Ложь);
